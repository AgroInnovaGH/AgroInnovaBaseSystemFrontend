...
import { buyingHeaderData, type BuyingColumn } from '../Data/buying_data';
...
const selectedColumns = ref<BuyingColumn[]>(buyingHeaderData);
const store = useRequestStore();
const { buyingData } = storeToRefs(store);

const filters = ref({
  global: { value: null, matchMode: FilterMatchMode.CONTAINS },
});

const onColumnToggle = (val: BuyingColumn[]) => {
  selectedColumns.value = buyingHeaderData.filter((col) => val.includes(col));
};
    ...
    <div class="table_card">
      ...
      <div class="datatable_wrapper">
        <DataTable
          ...
          :globalFilterFields="buyingHeaderData.map((col) => col.field)"
        >
          <Column
          ...
          >
            <template #body="{ data, field }">
              <template v-if="field === 'confirmed' || field === 'delivered'">
              ...
              <template v-else-if="field === 'transaction'">
                <div class="flex justify-center">
                  <Tag
                    :value="data[field as keyof typeof data]"
                    :severity="getSeverity(data[field as keyof typeof data] as string)"
              ...
              <template v-else-if="field === 'product.price'">
                <span class="currency">GHS</span> {{ data.product.price }}
              </template>
              <template v-else-if="field === 'product.total'">
                <span class="currency">GHS</span>
                <span class="font-bold text-emerald-700">{{ data.product.total }}</span>
              </template>
